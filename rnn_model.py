...
class LstmRNN(object):
    def __init__(self, sess, stock_count,
                 ...
                 keep_prob=0.8,
                 embed_size=None,
                 logs_dir=DEFAULT_CONFIG.log_dir,
    ...
    def build_graph(self):
        ...
        self.targets = tf.placeholder(
            tf.float32, [None, self.input_size], name="targets")
        ...
        if self.embed_size > 0:
            self.embed_matrix = tf.Variable(
                tf.random_uniform(
                    [self.stock_count, self.embed_size], -1.0, 1.0),
                name="embed_matrix"
            )
            sym_embeds = tf.nn.embedding_lookup(
                self.embed_matrix, self.symbols)
            ...
            stacked_embeds = tf.nn.embedding_lookup(
                self.embed_matrix, stacked_symbols)
        ...
        val, state_ = tf.nn.dynamic_rnn(
            cell, self.inputs, dtype=tf.float32, scope="dynamic_rnn")
        ...
        ws = tf.Variable(tf.truncated_normal(
            [self.lstm_size, self.input_size]), name="w")
        ...
        self.loss = tf.reduce_mean(
            tf.square(self.pred - self.targets), name="loss_mse")
        self.optim = tf.train.RMSPropOptimizer(
            self.learning_rate).minimize(self.loss, name="rmsprop_optim")
    ...
    def train(self, dataset_list, config):
        ...
        self.writer = tf.summary.FileWriter(
            os.path.join("./" + DEFAULT_CONFIG.log_dir, self.model_name))
            ...
            for label_, d_ in enumerate(dataset_list):
                for batch_X, batch_y in d_.generate_one_epoch(config.batch_size):
                    ...
                    self.writer.add_summary(
                        train_merged_sum, global_step=global_step)
        ...
        final_pred, final_loss = self.sess.run(
            [self.pred, self.loss], test_data_feed)
