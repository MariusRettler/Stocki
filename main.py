...
It handles all initialisations and it get used as the main execution script.
...
if not os.path.exists(DEFAULT_CONFIG.log_dir):
    os.mkdir(DEFAULT_CONFIG.log_dir)


def __show_all_variables():
    model_vars = tf.trainable_variables()
    slim.model_analyzer.analyze_vars(model_vars, print_info=True)


def __load_sp500(input_size, num_steps, k=None, target_symbol=None, test_ratio=0.05):
    ...
    info = pd.read_csv(DEFAULT_CONFIG.data_dir + "/constituents-financials.csv")
    info = info.rename(columns={col: col.lower().replace(' ', '_') for col in info.columns})
    info['file_exists'] = info['symbol'].map(lambda x: os.path.exists(DEFAULT_CONFIG.data_dir + "/{}.csv".format(x)))
    print(info['file_exists'].value_counts().to_dict())

    info = info[info['file_exists']].reset_index(drop=True)
    ...
    info[['symbol', 'sector']].to_csv(os.path.join(DEFAULT_CONFIG.log_dir + "/metadata.tsv"), sep='\t', index=False)
...
def main(_):
    ...
    with tf.Session(config=run_config) as sess:
        ...
        __show_all_variables()

        stock_data_list = __load_sp500(
            FLAGS.input_size,
            FLAGS.num_steps,
            k=FLAGS.stock_count,
